...
async def proxy_chat_completions(
            ...
            return JSONResponse(
                ...
                content={
                    "error": {
                        ...
                        "risk_level": prompt_risk.risk_level.value
                        if hasattr(prompt_risk.risk_level, "value")
                        else str(prompt_risk.risk_level),
                    }
                },
            )
    ...
    if not upstream_api_key:
        ...
        if db_key:
            try:
                ...
                logger.error(
                    f"Failed to decrypt key for user {current_user.id} provider {provider}"
                )
    ...
    try:
        if provider == "gemini":
            response_data = await _forward_to_gemini(
                request, upstream_api_key, settings.GEMINI_MODEL
            )
        elif provider == "groq":
            response_data = await _forward_to_groq(
                request, upstream_api_key, settings.GROQ_MODEL
            )
        elif provider == "gemini2":
            response_data = await _forward_to_gemini(
                request, upstream_api_key, settings.GEMINI2_MODEL
            )
        elif provider == "grok2":
            response_data = await _forward_to_groq(
                request, upstream_api_key, settings.GROK2_MODEL
            )
        elif provider == "openrouter":
        ...
        return JSONResponse(
            ...
            content={
                "error": {
                    "message": f"Upstream error: {e.response.text}",
                    "type": "upstream_error",
                }
            },
        )
        ...
        raise HTTPException(
            status_code=502, detail=f"Failed to reach upstream: {str(e)}"
        )
    ...
    try:
        ...
        if assistant_content:
                ...
                return JSONResponse(
                    ...
                    content={
                        "error": {
                            ...
                            "risk_level": output_risk.risk_level.value
                            if hasattr(output_risk.risk_level, "value")
                            else str(output_risk.risk_level),
                        }
                    },
                )
            ...
            response_data["intellectsafe"] = {
                ...
                "output_risk_level": output_risk.risk_level.value
                if hasattr(output_risk.risk_level, "value")
                else str(output_risk.risk_level),
            }
    except Exception as e:
...
async def _forward_to_groq(
    request: ChatCompletionRequest, api_key: str, model: str
) -> Dict[str, Any]:
...
async def _forward_to_openrouter(
    request: ChatCompletionRequest, api_key: str
) -> Dict[str, Any]:
...
async def list_models():
    ...
    return {
        ...
        "data": [
            ...
            {
                "id": "copilot-secure-bridge",
                "object": "model",
                "owned_by": "intellectsafe",
                "proxied_by": "intellectsafe",
            },
        ],
    }
